...
class UpLoadOthers extends Component{
    constructor() {
        super();
        this.state = {
            uploadValue: 0,
    ...
    async loadImages() {
        const q = query(collection(db, "otros"), orderBy("createdAt", "desc"));
    ...
    handleUpload = (event) => {
        ...
        const storageRef = ref(storage, `otros/${file.name}`);
        ...
        uploadTask.on(
            ...
            async () => {
                ...
                await addDoc(collection(db, "otros"), {
                    url: URL,
    ...
    handleDelete = async (imageId, imagePath) => {
        try {
            ...
            await deleteDoc(doc(db, "otros", imageId));
            this.loadImages();
        } catch (error) {
    ...
    handleEdit = async (imageId, updatedData) => {
        try {
            await updateDoc(doc(db, "otros", imageId), updatedData);
            this.loadImages();
        } catch (error) {
