...
class UpLoadNew extends Component {
    ...
    handleUpload = (event) => {
        ...
        const storageRef = ref(storage, `new-collection/${file.name}`);
        ...
        uploadTask.on(
            ...
            async () => {
                ...
                await addDoc(collection(db, "new-collection"), {
                    url: URL,
    ...
    handleDelete = async (imageId, imagePath) => {
        try {
            ...
            await deleteDoc(doc(db, "new-collection", imageId));
            this.loadImages();
        } catch (error) {
    ...
    handleEdit = async (imageId, updatedData) => {
        try {
            await updateDoc(doc(db, "new-collection", imageId), updatedData);
            this.loadImages();
        } catch (error) {
